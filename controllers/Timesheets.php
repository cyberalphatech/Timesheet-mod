...
class timesheets extends AdminController {
	...
	public function setting() {
		...
		if ($data['group'] == 'zkbio_time') {
			$data['tabs']['view'] = 'settings/zkbio_times/zkbio_time';
		} elseif ($data['group'] == 'zkbio_employee') {
			$data['tabs']['view'] = 'settings/zkbio_employees/zkbio_employee';
		}
 ...
 */
	public function requisition_detail($id) {
		...
		$request_leave = $this->timesheets_model->get_request_leave($id);
		if(!$request_leave){
			set_alert('warning', _l('ts_does_not_exist', _l('Leave')));

			redirect(admin_url('timesheets/requisition_manage'));
		}
		$data['request_leave'] = $request_leave;
	...
	public function show_detail_timesheets() {
		...
		if (!($value[0] == 'HO' || $value[0] == 'EB' || $value[0] == 'UB')) {
			...
			foreach ($list_shift as $ss) {
				...
				if ($data_shift_type) {
					$ws_day .= '<li class="list-group-item justify-content-between text-info bold">' . $data_shift_type->shift_type_name .'</li><li class="list-group-item justify-content-between">' . _l('work_times') . ': ' . $data_shift_type->time_start_work . ' - ' . ($data_shift_type->time_end_work != '00:00' ? $data_shift_type->time_end_work : '24:00') . '</li><li class="list-group-item justify-content-between">' . _l('lunch_break') . ': ' . $data_shift_type->start_lunch_break_time . ' - ' . $data_shift_type->end_lunch_break_time . '</li>';
				}
			}
 ...
 */
	...
	<div class="modal-body">';
		...
		<tr class="project-overview">
		<td class="bold">' . _l('timekeeping_type') . '</td>
		<td>' . ($additional_timesheets->timekeeping_type != '' ? _l($additional_timesheets->timekeeping_type) : _l('W')) . '</td>
		</tr>
		<tr class="project-overview">
		...
		<td>' . ($additional_timesheets->time_out != '00:00' ? $additional_timesheets->time_out : '24:00') . '</td>
		</tr>
		';
	...
	<div class="modal-footer">';
		...
		if (isset($check_approve_status['staffid'])) {
			if (in_array(get_staff_user_id(), $check_approve_status['staffid']) && $additional_timesheets->status == 0) {
			...
			if ($this->input->post()) {
				...
				if ($data_timesheet) {
					foreach ($rResult as $aRow) {
										...
										foreach ($list_tks as $key_tk => $tk) {
											...
											if (strtolower($split_val[0]) == 'w') {
												if (isset($split_val[1]) && is_numeric($split_val[1]) && $shift_hour > 0) {
													...
													$total += $cal;
													$data_working_hour[$date] = $this->timesheets_model->get_total_working_hour($aRow['staffid'], $date);
												}
											}

											if (strtolower($split_val[0]) == 'ot') {
												if (isset($split_val[1]) && is_numeric($split_val[1]) && $shift_hour > 0) {
													$cal = $split_val[1] / $shift_hour;
													$total1 += $cal;
													$data_working_hour[$date] = $this->timesheets_model->get_total_working_hour($aRow['staffid'], $date);
						...
						$row[] = ($total1 > 0) ? (float) number_format($total1, 2) : 0;
						...
						$total_data_working_hour = 0;
						foreach ($data_working_hour as $value) {
							$total_data_working_hour += $value;
						}
						$row[] = number_format($total_data_working_hour, 2);
						$output['aaData'][] = $row;
					}
				}
				echo json_encode($output);
 ...
 */
	public function table_shiftwork() {
		...
		$data_hs = $this->set_col_tk($date, date('Y-m-t'), $month, $month_year, true, $list_staff_id);
		...
		foreach ($list_staff_id as $key => $value) {
			...
			foreach ($list_date as $kdbm => $day) {
				...
				$day_s = date('M d - D', strtotime($day));
 ...
 */
		...
		if ($this->input->is_ajax_request()) {
			if ($this->input->post()) {
				...
				if (!has_permission('table_shiftwork_management', '', 'view') && !is_admin()) {
					$where[] = 'AND ' . db_prefix() . 'shift_type.add_from = '.get_staff_user_id();
				}
	...
	public function set_col_tk($from_day, $to_day, $month, $month_year, $absolute_type = true, $stafflist = '', $work_shift_id = '') {
		...
		if ($absolute_type == true) {
			...
			$list_date = $this->timesheets_model->get_list_date($from_day, $to_day);

			foreach ($list_date as $key => $date) {
				$time = strtotime($date);
				array_push($data_time, $time);
				array_push($data_day_by_month_tk, date('M d - D', $time));
				array_push($data_day_by_month, date('M d - D', $time));
				array_push($data_set_col, ['data' => date('M d - D', $time), 'type' => 'text']);
				array_push($data_set_col_tk, ['data' => date('M d - D', $time), 'type' => 'text']);

				array_push($data_set_col_tk, ['data' => date('M d - D', $time), 'type' => 'text']);
				array_push($list_data, [
					'data' => date('M d - D', $time),
					'editor' => "chosen",
					'chosenOptions' => [
						'data' => $new_list_shift,
					],
 ...
 */
	public function add_allocation_shiftwork($id = '') {
		...
		$month = date('m');
		$month_year = date('Y');
		$data_hs = $this->set_col_tk($month_year.'-'.$month.'-01',$month_year.'-'.$month.'-08', $month, $month_year, false);
		...
		foreach ($data_shift_type as $key => $value) {
			...
			array_push($new_list_shift, array('key' => $value['id'],
				'value' => $value['shift_type_name'] . ' (' . $st_time . ' - ' . $e_time . ')',
		...
		if ($id != '') {
			...
			if ($data['word_shift']->type_shiftwork == 'repeat_periodically') {
				$data_hs = $this->set_col_tk($month_year.'-'.$month.'-01',$month_year.'-'.$month.'-08', $month, $month_year, false, $staff, $id);
				$data['head_data'] = $data_hs->day_by_month;
				$data['list_data'] = $data_hs->list_data;
				$data['data_object'] = $data_hs->data_object;
			}
			...
			if ($data['word_shift']->type_shiftwork == 'by_absolute_time') {
				...
				$data_hs = $this->set_col_tk($data['word_shift']->from_date, $data['word_shift']->to_date, $month, $month_year, true, $staff, $id);
				$data['head_data'] = $data_hs->day_by_month;
				$data['list_data'] = $data_hs->list_data;
				$data['data_object'] = $data_hs->data_object;
			}
 ...
 */
	function get_hanson_shiftwork() {
		...
		if ($type_shiftwork == 'repeat_periodically') {
			$data_hs = $this->set_col_tk($month_year.'-'.$month.'-01',$month_year.'-'.$month.'-08', $month, $month_year, false, $staff);
			echo json_encode([
		...
		if ($type_shiftwork == 'by_absolute_time') {
			...
			$data_hs = $this->set_col_tk($from_date, $to_date, $month, $month_year, true, $staff);
			echo json_encode([
				'head_data' => $data_hs->day_by_month,
				'list_data' => $data_hs->list_data,
				'data_object' => $data_hs->data_object,
	...
	function shift_table() {
		if ($this->input->is_ajax_request()) {
			if ($this->input->post()) {
				...
				if(!is_admin() && !has_permission('table_shiftwork_management', '', 'view')){
					$shift_ids = [];
					$list_shift = $this->timesheets_model->get_shift_ids_by_staff(get_staff_user_id());

					$shift_ids = implode(',', array_values($list_shift));

					if($shift_ids != ''){
						$where[] = 'AND (' . db_prefix() . 'work_shift.add_from = '.get_staff_user_id().' OR ' . db_prefix() . 'work_shift.id IN ('.$shift_ids.'))';
					}else{
						$where[] = 'AND ' . db_prefix() . 'work_shift.add_from = '.get_staff_user_id();
					}
				}
				$aColumns = $select;
				$sIndexColumn = 'id';
				...
				foreach ($rResult as $aRow) {
					...
					$option = '';
					if (has_permission('table_shiftwork_management', '', 'view') || $aRow['add_from'] == get_staff_user_id() || is_admin()) {
 ...
 */
	public function reload_shiftwork_byfilter() {
		...
		$data_hs = $this->set_col_tk($month_year.'-'.$month.'-01',date('Y-m-t', strtotime($month_year.'-'.$month.'-01')) , $month, $month_year, true, $list_staff_id);
	...
	public function check_in_out_progress_report() {
		if ($this->input->is_ajax_request()) {
			if ($this->input->post()) {
				...
				$department_fillter = $this->input->post("department_2_fillter");
				$role_fillter = $this->input->post("roles_2_fillter");
				$type_fillter = $this->input->post("type_22_fillter") ?? 3;
...
// Workplace
		...
		$this->db->where('days_off > 0');
		$timesheets_day_off = $this->db->get(db_prefix() . 'timesheets_day_off')->result_array();
		$day_off_arr = [];
		foreach ($timesheets_day_off as $key => $value) {
			$node = $value;
			$node['days_off'] = 0;
			$node['remain'] = $node['total'];
			$day_off_arr[] = $node;
		}

		$this->db->update_batch(db_prefix() . 'timesheets_day_off', $day_off_arr, 'id');
					...
					if ($aRow['position']) {
						if ($aRow['position'] != '') {
							...
							foreach ($list_position as $key => $value) {
								$aRowata_role = $this->timesheets_model->get_role($value);
								if ($aRowata_role) {
 ...
 */
	...
	/**
	 * attendance by department report
	 * @return json
	 */
	public function attendance_by_department() {
		echo json_encode($this->timesheets_model->report_attendance_by_department());
	}

	/**
	 * attendance by workplace report
	 * @return json
	 */
	public function attendance_by_workplace() {
		echo json_encode($this->timesheets_model->report_attendance_by_workplace());
	}

	/**
	 * absences by department report
	 * @return json
	 */
	public function absences_by_department() {
		echo json_encode($this->timesheets_model->report_absences_by_department());
	}

	/**
	 * absences by workplace report
	 * @return json
	 */
	public function absences_by_workplace() {
		echo json_encode($this->timesheets_model->report_absences_by_workplace());
	}

	public function add_timesheet_leave($id) {
		$this->timesheets_model->add_timesheet_leave('SI', $id);
	}
	 ...
	 * @return void
	 */
    public function zkbio_time_table()
    {
        $this->app->get_table_data(module_views_path('timesheets', 'settings/zkbio_times/zkbio_time_table'));
    }

	/**
	 * setting zkbio time 
	*/
	public function setting_zkbio_time()
	{
		$data = $this->input->post();
		if($data != 'null'){
			$value = $this->timesheets_model->update_setting_zkbio_time($data);
			if($value){
				$success = true;
	...
	/**
	 * zkbio employee table
	 * @return void
	 */	
	public function zkbio_employee_table()
    {
        $this->app->get_table_data(module_views_path('timesheets', 'settings/zkbio_employees/zkbio_employee_table'));
    }

	/**
	 * Summary of zkbio mapping timekeeping
	 * @return never
	 */
	public function zkbio_mapping_timekeeping()
	{
		$message = '';
		$success = false;
		$success = $this->timesheets_model->xmzkteco_all_transactions();
		if ($success) {
			$message = _l('ts_updated_successfully', _l('ts_zkbio_mapping_employee'));
		}else{
			$message = _l('ts_There_is_no_new_data_to_sync.');
		}
	...
	/**
	 * zkbio mapping employee
	 */
	public function zkbio_mapping_employee() {
		$message = '';
		$success = false;

		$response = $this->timesheets_model->xmzkteco_get_employees();
	...
	/**
	 * zkbio mapping employee manual
	 */
	public function zkbio_mapping_employee_manual()
	{
		$this->db->where('id', $this->input->post('emp_id'));
		$this->db->update(db_prefix() . 'timesheets_kteco_employees', ['perfex_staff_id' => $this->input->post('assignee')]);
	...
	/**
	 * remove mapping employee
	 */
	public function remove_mapping_employee($id, $taskid)
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'timesheets_kteco_employees', ['perfex_staff_id' => 0]);
	...
	{
		$data = $this->input->post();
		$success = $this->timesheets_model->cal_timesheet_hours($data['timesheet_month']);
		if ($success) {
			$message = _l('added_successfully', _l('ts_cal_timesheet_hours'));
			set_alert('success', $message);
		}
		redirect(admin_url('timesheets/setting?group=zkbio_time'));
	}

	/**
	 * zkbio time synch table
	 * @return void
	 */
	public function zkbio_time_synch_table()
    {
        $this->app->get_table_data(module_views_path('timesheets', 'settings/zkbio_times/zkbio_time_synch_table'));
    }
