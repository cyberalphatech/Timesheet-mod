<script>
	...
	function init_report(e, type) {
		...
		$('.select_all_time').removeClass('hide');
		...
		$('.attendance_by_department').addClass('hide');
		$('.attendance_by_workplace').addClass('hide');
		$('.absences_by_department').addClass('hide');
		$('.absences_by_workplace').addClass('hide');
		...
		if(type == 'working_hours'){
			...
			$('#report-time').removeClass('hide');
		} else if(type == 'attendance_by_department'){
			$('.select_all_time').addClass('hide');
			$('select[name="months-report"]').selectpicker('refresh');
			$('.attendance_by_department').removeClass('hide');
			$('.staff_2_fr').removeClass('hide');
			$('.department_2_fr').removeClass('hide');
			$('.sorting_2_table').removeClass('hide');
			$('#report-time').removeClass('hide');
		} else if(type == 'attendance_by_workplace'){
			$('.select_all_time').addClass('hide');
			$('select[name="months-report"]').selectpicker('refresh');

			$('.attendance_by_workplace').removeClass('hide');
			$('.workplace_2_fr').removeClass('hide');
			$('.staff_2_fr').removeClass('hide');
			$('.sorting_2_table').removeClass('hide');
			$('#report-time').removeClass('hide');
		} else if(type == 'absences_by_department'){
			$('.absences_by_department').removeClass('hide');
			$('.department_2_fr').removeClass('hide');
			$('.staff_2_fr').removeClass('hide');
			$('.sorting_2_table').removeClass('hide');
			$('#report-time').removeClass('hide');
		} else if(type == 'absences_by_workplace'){
			$('.absences_by_workplace').removeClass('hide');
			$('.workplace_2_fr').removeClass('hide');
			$('.staff_2_fr').removeClass('hide');
			$('.sorting_2_table').removeClass('hide');
			$('#report-time').removeClass('hide');
	 ...
	 function gen_reports() {
	 	...
	 	if(current_type != ''){
	 		switch(current_type){
	 			...
	 			break;
	 			case 'attendance_by_workplace':
	 			attendance_by_workplace();
	 			break;
	 			case 'absences_by_department':
	 			absences_by_department();
	 			break;
	 			case 'absences_by_workplace':
	 			absences_by_workplace();
	 			break;
	 		}
	 	}
	 }
	...
	function attendance_by_department() {
		"use strict";
		if (typeof(groupsChart) !== 'undefined') {
			groupsChart.destroy();
		}
		var data = {};
		data.months_report = $('select[name="months-report"]').val();
		data.departments = $('select[name="department_2_fillter[]"]').val();
		data.staffs = $('select[name="staff_2_fillter[]"]').val();
		data.report_from = report_from.val();
		data.report_to = report_to.val();


		$.post(admin_url + 'timesheets/attendance_by_department', data).done(function(response) {
			response = JSON.parse(response);
			 //get data for hightchart
			 Highcharts.setOptions({
			 	chart: {
			 ...
			 Highcharts.chart('attendance_by_department', {
			 	chart: {
			 		type: 'column'
			 	},
			 	title: {
			 		text: '<?php echo _l('attendance_by_department'); ?>'
			 	},
			 	credits: {
			 		enabled: false
			 	},
	...
	function attendance_by_workplace() {
		"use strict";
		if (typeof(groupsChart) !== 'undefined') {
			groupsChart.destroy();
		}
		var data = {};
		data.months_report = $('select[name="months-report"]').val();
		data.workplaces = $('select[name="workplace_2_fillter[]"]').val();
		data.staffs = $('select[name="staff_2_fillter[]"]').val();
		data.report_from = report_from.val();
		data.report_to = report_to.val();


		$.post(admin_url + 'timesheets/attendance_by_workplace', data).done(function(response) {
			response = JSON.parse(response);
			 //get data for hightchart
			 Highcharts.setOptions({
			 	chart: {
			 ...
			 Highcharts.chart('attendance_by_workplace', {
			 	chart: {
			 		type: 'column'
			 	},
			 	title: {
			 		text: '<?php echo _l('attendance_by_workplace'); ?>'
			 	},
			 	credits: {
			 		enabled: false
			 	},
	...
	function absences_by_department() {
		"use strict";
		if (typeof(groupsChart) !== 'undefined') {
			groupsChart.destroy();
		}
		...
		$.post(admin_url + 'timesheets/absences_by_department', data).done(function(response) {
			response = JSON.parse(response);
			 //get data for hightchart
			 Highcharts.setOptions({
			 	chart: {
			 ...
			 Highcharts.chart('absences_by_department', {
			 	chart: {
			 		type: 'column'
			 	},
			 	title: {
			 		text: '<?php echo _l('absences_by_department'); ?>'
			 	},
			 	credits: {
			 		enabled: false
			 	},
	...
	function absences_by_workplace() {
		"use strict";
		if (typeof(groupsChart) !== 'undefined') {
			groupsChart.destroy();
		}
		...
		$.post(admin_url + 'timesheets/absences_by_workplace', data).done(function(response) {
			response = JSON.parse(response);
			 //get data for hightchart
			 Highcharts.setOptions({
			 	chart: {
			 ...
			 Highcharts.chart('absences_by_workplace', {
			 	chart: {
			 		type: 'column'
			 	},
			 	title: {
			 		text: '<?php echo _l('absences_by_workplace'); ?>'
			 	},
			 	credits: {
			 		enabled: false
			 	},
