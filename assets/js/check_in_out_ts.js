  ...
  scan_session = setTimeout(function() {
    try {
      ...
      if (split_result2[0] != '') {
        ...
        var data = {};
        data.location_user = $('input[name="location_user"]').eq(0).val();
        data.point_id = $('#route_point').val();

        $.post(admin_url + 'timesheets/checkin_from_qr_code/'+user_id, data).done(function (response) {
          response = JSON.parse(response);
