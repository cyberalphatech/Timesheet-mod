...
<script>
  ...
  var latitudeBase = 0.0;
  var longitudeBase = 0.0;
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        function (position) {
          latitudeBase = position.coords.latitude;
          longitudeBase = position.coords.longitude;
        }
      );
    }
      ...
      function initMap() {
        ...
        if(typeof flightPlanCoordinates[0] != 'undefined'){
          ...
          console.log('latitudeBase: '+latitudeBase);
          console.log('longitudeBase: '+longitudeBase);
           var map = new google.maps.Map(document.getElementById("map"), {
          zoom: 17,
          center: {lat:latitudeBase, lng: longitudeBase},
        });       
       }
     }
