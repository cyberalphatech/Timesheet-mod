...
define('TIMESHEETS_REVISION', 123);
...
function timesheets_cron_approval() {
	...
	foreach ($approval_overdue as $k => $val) {
		...
		$CI->db->where('id', $val['rel_id']);
		$CI->db->update(db_prefix() . 'timesheets_requisition_leave', [
			'status' => '2',
		]);
...
function timesheets_appint(){
    ...
    if(!$timesheets_gtssres || ($timesheets_gtssres && isset($timesheets_gtssres['status']) && !$timesheets_gtssres['status'])){
        ...
        set_alert('danger', timesheet_decrypt('plGJBtUappDE3nHKyVHLrW7/hP1hS3GF2gj8p2o/ZLC5ETPQIs2GtJLQ1eOaoWXUBkF/F/RnkOiHDxsFxIAzAQlGrUZAfVufkpsftfXeGYKDHJWgpXR0Flf6Tw1G123yLoumNQnMbhRW2RPdhagn7A+KxbS8QEG0NzP8I0zFUG/ZG+V6oG4OTdETpxFBRQincNmA960Lj/a4aDuS1Hj2Hw=='));
...
function after_render_top_search_timesheets() {
	...
	if ((has_permission('attendance_management', '', 'view_own') || has_permission('attendance_management', '', 'view') || is_admin())) {
		...
		if ($data_timekeeping_form == 'timekeeping_manually') {
			...
			<a href="#" class="check_in_out_timesheet" data-toggle="tooltip" title="" onclick="open_check_in_out();" data-placement="bottom" data-original-title="' . _l('check_in') . ' / ' . _l('check_out') . '"><i class="fa fa-history fa-fw "></i>
